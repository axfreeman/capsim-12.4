...
# Clone all classes in this simulation
    ...
    for stock in stocks:
        ...
        new_stock:Industry_stock = clone_model(stock, db)
        ...
        new_stock.commodity_name=new_stock.commodity(db).name+"(sim "+new_stock.commodity(db).simulation_id+")"
        new_stock.industry_name=new_stock.new_stock.industry(db).name+"(sim "+new_stock.industry(db).simulation_id+")"
    ...
    for stock in stocks:
        ...
        new_stock:Class_stock = clone_model(stock, db)
        db.add(new_stock)  # we commit twice, because we want to get the autogenerated id. There's probably a better way
        new_stock.simulation_id = new_simulation.id
        # This stock now has to be connected with its owner and commodity objects in the new simulation
        ...
        new_stock.commodity_id = successor_commodity_id
        new_stock.commodity_name=new_stock.commodity(db).name+"(sim "+new_stock.commodity(db).simulation_id+")"
        new_stock.industry_name=new_stock.new_stock.social_class(db).name+"(sim "+new_stock.social_class(db).simulation_id+")"
